...
def post_question_shown(
    payload: ShownQuestion, memory: ConversationBufferMemory = Depends(MemoryDep)
):
...
def post_user_answer(
    payload: UserAnswer, memory: ConversationBufferMemory = Depends(MemoryDep)
):
    log_user_answer(
        memory, payload.question_id, payload.answer, payload.answer_duration_sec
    )
    return {"ok": True}

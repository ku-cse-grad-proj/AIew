...
class FollowupRequest(BaseModel):
    ...
    userAnswer: str = Field(..., description="사용자 답변 전문")
    ...
    model_config = {
        "json_schema_extra": {
            "example": {
                ...
                "userAnswer": "API 성능 최적화를 위해 여러 가지 접근을 시도했습니다. 먼저 데이터베이스 쿼리를 분석해 불필요한 풀스캔을 없애고, 필요한 곳에 인덱스를 추가했습니다. 또한 Redis를 활용해 자주 조회되는 데이터를 캐싱하여 응답 속도를 크게 줄였습니다. 네트워크 측면에서는 gzip 압축과 HTTP/2를 적용해 전송 효율을 높였고, 서버 운영 환경에서는 PM2 클러스터 모드로 멀티코어를 활용하며 무중단 배포를 구현했습니다. 마지막으로 New Relic을 이용해 성능 지표를 실시간 모니터링하며, 병목 구간이 발견되면 즉시 개선 작업을 진행했습니다.",
...
class FollowupResponse(BaseModel):
    ...
    questionText: str = Field(..., alias="question", description="꼬리질문 본문")

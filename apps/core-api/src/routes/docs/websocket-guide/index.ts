  ...
  const opts: RouteShorthandOptions = {
    schema: {
      ...
      description:
        ...
        '<h3>통신 순서</h3>' +
        '1.  **연결 수립**: 클라이언트는 `POST /api/v1/interviews` API를 호출하여 면접 세션을 생성하고 `sessionId`를 받습니다. Next.js와 같은 클라이언트 환경에서는 `socket.io-client` 라이브러리를 사용하여 서버에 연결합니다. 연결 시 `query` 옵션에 `sessionId`를 포함시켜야 합니다. <pre><code>const socket = io("http://localhost:3000", { query: { sessionId } });</code></pre>' +

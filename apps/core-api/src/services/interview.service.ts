...
export class InterviewService {
  ...
  public async createInterviewSession(
    ...
    log.info(`[${sessionId}] Starting interview creation process...`)
    ...
    const uploadPromises = Object.entries(files).map(async ([key, file]) => {
      ...
      // as const를 사용하여 key의 타입을 'coverLetter' | 'portfolio'로 좁힙니다.
      return {
    ...
    const questionRequestData = {
      ...
      // constraints는 ai-server의 기본값을 사용합니다.
    }
    ...
    // 4. DB에 세션 저장
    log.info(`[${sessionId}] Saving interview session to DB...`)
    ...
    log.info(`[${sessionId}] Interview session saved. ID: ${session.id}`)

    // 5. 생성된 질문들을 DB에 저장 (백그라운드 실행)
    this.saveQuestionsAndNotifyClient(session.id, generatedQuestions)
